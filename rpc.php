...
session_start();
	...
	case 'eventList':
		...
		if($eventQuery) {
			...
			?>
			<span style="text-align: right; position: relative; margin-left: 410px; top: -20px; cursor: pointer;" onClick="showEventForm( <?=$day?>,<?=$month?>, <?=$year?> )">
				<img src="images/nuevo_evento.png" title="Crear Nuevo Evento" alt="Crear Nuevo Evento"/></span>
			<? }else{ ?>
			<span style="text-align: right; position: relative; margin-left: 340px; cursor: pointer;"><br/><br/>&nbsp;</span>

			<? } ?>
			<div style="margin-left: 45px; margin-top: -90px;">
				<div style="background: url(images/dia_evento.jpg) no-repeat; position: relative; font: 15px Verdana, Arial; text-weight: bold; line-height: 20px; color: black; width: 300px; height: 140px;">
				<b><br/><br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?=$day?>/<?=$month?>/<?=$year?></b></div>
				<?
				...
				for($i=0; $i<mysql_num_rows($eventQuery); $i++) {
					...
					$msg = mysql_fetch_array($eventQuery);
						...
						?>
						<div style="background:<?=$textColor?>; position:relative; top: -60px; padding: 1px; margin-right: 20px;" id="event_<?=$msg['evento_id']?>">
						...
						<img alt="Borrar" title="Borrar" src="images/borrar.png" style="width:15px;height:15px;"/>	</span>
						<?}?></div>
						...
						if($msg['evento_horafin'] == $msg['evento_horainicio'] && ( $msg['evento_horafin'] == "00:00:00" || $msg['evento_horafin'] == NULL ) ){
							...
							if($msg['evento_horafin'] != "00:00:00" || $msg['evento_horafin'] != NULL)	echo " y finaliza a las " . $pieceshasta[0]. ":". $pieceshasta[1]  . "hs.";
