...
if(isset($_REQUEST["return_url"]) && $_REQUEST["return_url"] != "")
	...
	if ( GetParameter("btnActualizar") != "" ) 
	{
		if ( GetParameter("action") == "new" && $MsgBoxMessage == "" )
		{
			...
			$strSQLComm = " INSERT INTO facultad(facultad_titulo, universidad_id, activo) " .
							" VALUES (" .
							"'" . SafeSql(GetParameter("txtFacultad")) . "'," . 
							SafeSql(GetParameter("cmbUniversidad")) . "," .
							" 'Y'" .
							")";
			...
			if(mysql_error() != ""){
				...
				$MsgBoxMessage = "No se pudo crear la facultad, por favor inténtelo nuevamente.";
			}
...
<script language="JavaScript" type="text/javascript">
	function validarForm(thisForm) {
		...
		if(thisForm.txtFacultad.value == ''){
			alert('Debe ingresar el título de la facultad.');
			thisForm.txtFacultad.focus();
			return false;
		}
...
<form action="<?=$_SERVER["SCRIPT_NAME"]?>" method="post" name="frmForm" onsubmit="return validarForm(this);">
	...
	<div class="FormularioPopUp" >
		<div class="separaModulo">
			...
			<div id="dvUniversidades"><? ObtenerComboUniversidad("", decrypt(GetParameter("idu")), "", "--Seleccione--", "Y", "AcomodaDdlFiltroGrande")?></div>
			<div class="AcomodaLabel">
				T&iacute;tulo de la Facultad: (*)
			</div>
			<input type="text" id="txtFacultad" name="txtFacultad" value="<?=GetParameter("txtFacultad")?>" maxlength="255" style="width:254px;"/>
	</div>
