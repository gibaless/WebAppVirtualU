...
$strSQLCommMensajes = " SELECT DISTINCT mensaje_id, mensaje_activo, mensaje_fecha, mensaje_titulo, mensaje_mensaje, mensaje_tipo, usuario_id, usuario_nombre, usuario_apellido, " . 
				...
				" INNER JOIN mensaje_usuario AS MU ON (M.mensaje_id = MU.mensaje_id AND MU.usuario_id = " . SafeSql($_SESSION["UserId"]) . ") " . //Mensajes Privados. 
				" )UNION( " .
				" SELECT M.mensaje_id, M.mensaje_activo, M.mensaje_fecha, M.mensaje_titulo, M.mensaje_mensaje, M.mensaje_tipo, U.usuario_id, U.usuario_nombre, U.usuario_apellido, " . 
...
if(GetParameter("txtNombre") != ""){
	$strSQLCommMensajes = $strSQLCommMensajes . " AND LCASE(CTOTAL.usuario_nombre) LIKE '%" . strtolower(SafeSql(GetParameter("txtNombre"))) . "%'";
}
if(GetParameter("txtApellido") != ""){
	$strSQLCommMensajes = $strSQLCommMensajes . " AND LCASE(CTOTAL.usuario_apellido) LIKE '%" . strtolower(SafeSql(GetParameter("txtApellido"))) . "%'";
}

$strSQLCommMensajes = $strSQLCommMensajes . " ORDER BY CTOTAL.mensaje_fecha DESC ";
//echo($strSQLCommMensajes);exit();
// ******** PAGINADOR
$PagRowsPerPage = 20;
...
include "header.php"; 
?>
<h2>Mis Mensajes (Bandeja de entrada)</h2>
<div class="SpaceFiltroBusquedaBig">
<p class="NombreFiltro">Filtro de Búsqueda</p><hr class="HRFiltro" align="left"/>
<form action="<?=$_SERVER["SCRIPT_NAME"]?>" method="post" name="frmForm" id="frmForm">
	...
	<div class="botonesFiltro">
		...
		<input class="botonFiltro" type="button" value="Cancelar" onclick="self.location='mismensajes.php';" name="btnVolver">
	</div>
</form>
...
function Denunciar(sId){
		window.open('denunciar.php?t=M&id=' + sId,'denunciar','toolbar=no,menubar=no,resizable=no,scrollbars=yes,width=490,height=350, left=100, top=100');
}
</script>
	...
	<div class="SubMenu">
		...
		<a class="boton" href="mismensajesenviados.php">Mensajes Enviados</a>
			...
			<table border="0" cellspacing="0" cellpadding="0">
				...
				if(mysql_num_rows($Result)){
					...
					while($oRs = mysql_fetch_array($Result)){
						...
						$strCreado = DateTimeFormat($oRs["mensaje_fecha"]) . "<br/>por <strong>" . $oRs["usuario_nombre"] . " " . $oRs["usuario_apellido"] . "</strong>";
						$strGrupo = $oRs["grupo"];
...
<a href="mensajever.php?id=<?=encrypt($oRs["mensaje_id"])?>&action=rec&return_url=<?=$strReturnUrl?>">
<img src="images/ver_mensaje.png" title="Ver Mensaje" alt="Ver Mensaje" style="width: 30px; border:0;"/></a>
<a href="mensajeform.php?id=<?=encrypt($oRs["mensaje_id"])?>&action=new&return_url=<?=$strReturnUrl?>">
<img src="images/message_reply.png" title="Responder" alt="Responder" style="width: 28px; border:0;"/></a>
<a href="javascript://;" onclick="Denunciar('<?=encrypt($oRs["mensaje_id"])?>');">Denunciar</a>
							<? }?>
							</td>
						</tr>
				<?	}
