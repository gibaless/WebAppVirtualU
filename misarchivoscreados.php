...
if(GetParameter("id") != "" && is_numeric(decrypt(GetParameter("id")))){
	$iIdArchivo = decrypt(GetParameter("id"));
}
...
//Borro el archivo creado por el usuario
if(GetParameter("btnBorrar") == "yes"){
	$strSQLComm = " UPDATE archivo SET " .
				  " activo = 'N' " . 
				  " WHERE archivo_id = " . SafeSql($iIdArchivo) .
...
if(GetParameter("txtTitulo") != ""){
	$strSQLCommArchivos = $strSQLCommArchivos . " AND (LCASE(CTOTAL.archivo_titulo) LIKE '%" . strtolower(SafeSql(GetParameter("txtTitulo"))) . "%'" .
						  " OR LCASE(CTOTAL.archivo_archivo) LIKE '%" . strtolower(SafeSql(GetParameter("txtTitulo"))) . "%')";
}
if(GetParameter("txtNombre") != ""){
	$strSQLCommArchivos = $strSQLCommArchivos . " AND LCASE(CTOTAL.usuarioenviado_nombre) LIKE '%" . strtolower(SafeSql(GetParameter("txtNombre"))) . "%'";
}
if(GetParameter("txtApellido") != ""){
	$strSQLCommArchivos = $strSQLCommArchivos . " AND LCASE(CTOTAL.usuarioenviado_apellido) LIKE '%" . strtolower(SafeSql(GetParameter("txtApellido"))) . "%'";
}
...
<p class="NombreFiltro">Filtro de Búsqueda</p><hr class="HRFiltro" align="left"/>
	...
	<div class="botonesFiltro">
		...
		<input class="botonFiltro" type="button" value="Cancelar" onclick="self.location='misarchivoscreados.php';" name="btnVolver">
	</div>
	</div>
...
<div class="SubMenu">
<a class="boton" href="misarchivos.php">Mis Archivos Recibidos</a>
	...
	if(mysql_num_rows($Result)){
		...
		while($oRs = mysql_fetch_array($Result)){
			...
			<tr class="<? if ($ii % 2  != 0){ ?>RowStyle<? }else{?>AltRowStyle<? }?>">
				...
				<td class="TDright">
				<? if($oRs["activo"] != "P"){?>
				<a href="archivover.php?id=<?=encrypt($oRs["archivo_id"])?>&action=env&return_url=<?=$strReturnUrl?>"><img src="images/lupa.png" title="Ver Detalles" style="width:20px;"/></a>
				<a  href="archivoform.php?id=<?=encrypt($oRs["archivo_id"])?>&action=edit&return_url=<?=$strReturnUrl?>"><img src="images/lapiz.png" title="Editar" style="width:20px;"/></a> 
					<a href="javascript:ConfirmDelete('misarchivoscreados.php?btnBorrar=yes&id=<?=encrypt($oRs["archivo_id"])?>&return_url=<?=urlencode($strReturnUrl)?>', 'Desea borrar el archivo?');"><img src="images/trash_empty.png" title="Borrar" style="width:20px;"/></a>
				<? }?>
				</td>
			</tr>
	<?	}
